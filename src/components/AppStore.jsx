...
const AppStore = () => {
  ...
  let filteredAppsList = appsList.filter((app) => app.category === activeTabId);

  if (searchInput !== "") {
    filteredAppsList = filteredAppsList.filter((app) => {
      return app.appName.toLowerCase().includes(searchInput.toLowerCase());
    });
  }
  ...
  return (
    <div className="flex min-h-screen flex-col justify-center items-center bg-gradient-to-b from-[#fff1eb] to-[#ace0f9] font-serif">
      <h1 className="text-4xl mb-8 font-bold">App Store</h1>
      <input
        type="search"
        className="mb-8 p-2 w-[300px] rounded-lg"
        placeholder="Search"
        name="search"
      ...
      <div className="w-1/2">
        <ul className="flex list-none justify-center items-center flex-wrap">
